...
public class ModifierAdapter extends RecyclerView.Adapter<ModifierAdapter.eiewHolder>
{
	private Context mContext;
	private List<Bean> mDatas;
	public ModifierAdapter(Context context, List<Bean> datas){
		mContext=context;
		mDatas=datas;
	}
	...
	public void onBindViewHolder(final eiewHolder p1, final int p2){/**p2位索引**/
		...
		if(bean.gettitle()!=null && !bean.gettitle().equals("") && bean.getid()!=null && !bean.getid().equals("")){
			...
			p1.ship_image.setImageResource(R.drawable.image_rocket);
					...
					new AlertDialog.Builder(mContext)
							...
							.setNegativeButton("确定", new DialogInterface.OnClickListener() {
								...
								public void onClick(DialogInterface dialog, int which) {
									File file=new File(MainActivity.path+"ships/"+bean.gettitle());
									file.delete();
	...
	private ModifierAdapter.onHistoryClick onClick;
	//onclick
